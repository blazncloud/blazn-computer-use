...
| Inert scroll probe | `inert-scroll-probe` | AXScrollArea | fixed probe rows + `scroll-churn` timer | `scroll` false-success regression: the target consumes wheel events but never moves content while an unrelated timer changes the tree. A correct verifier reports `effect_not_verified` even though a naive whole-window-diff check would see `renderedTextChanged`. |
